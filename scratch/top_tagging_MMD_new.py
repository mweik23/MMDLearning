...
def display_status(res, num_batches, partition, epoch, batch_num, batch_size):
    ...
    tmp_BCE_loss = src.sum_reduce(res['BCE loss'], device = local_rank) / tmp_counter
    tmp_MMD_loss = src.sum_reduce(res['MMD loss'], device = local_rank) / tmp_counter
...
def run(epoch, loaders, partition):
    ...
    for i, data in enumerate(zip(*loaders)):
        ...
        output = [get_pred(d) for d in data]
...
if __name__ == "__main__":
    ...
    model = LorentzNet(n_scalar = 2, n_hidden = args.n_hidden, n_class = 2,
                       ...
                       c_weight = args.c_weight, no_batchnorm=args.no_batchnorm)
    ...
    if args.lr_scheduler=='CosineAnealing':
        ...
        base_scheduler = ReduceLROnPlateau(optimizer) 
    else:
    ...
    if args.pretrained != '' and not args.test_mode:
        start_epoch = load_ckp(f"{args.logdir}/{args.pretrained}.pt", ddp_model, optimizer=optimizer)
