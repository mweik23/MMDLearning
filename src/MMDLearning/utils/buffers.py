...
class EpochLogitBuffer:
    ...
    def clear(self):
        self._logits.clear(); self._labels.clear()
    ...
    def _stitch_local(self) -> Dict[str, torch.Tensor]:
        dev = self._logits[0].device if self._logits else torch.device("cpu")
        out = {
            "logits": torch.cat(self._logits, dim=0) if self._logits else torch.empty(0, device=dev),
    ...
    def gather_to_rank0(self, *, cast_fp16: bool = True, reorder_by_indices: bool = False) -> Optional[Dict[str, torch.Tensor]]:
        ...
        logits = local["logits"]; labels = local["labels"]
        n_local = int(labels.shape[0])

        # Choose gather path
        if self.assume_equal_lengths:
            logits_all = self._gather_equal(logits)
            ...
            logits_all = self._gather_varlen(logits, n_local)
        ...
        if cast_fp16:
            logits_all = logits_all.to(torch.float16)
        payload = {
            "logits": logits_all.cpu(),
        ...
        if indices_all is not None:
            ...
            if reorder_by_indices and idx_cpu.numel() > 0:
                ...
                payload["logits"] = payload["logits"][order]
