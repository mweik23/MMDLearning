...
class LGEB(nn.Module):
    def __init__(self, n_input, n_output, n_hidden, n_node_attr=0,
                 dropout = 0., c_weight=1.0, last_layer=False, no_batchnorm=True, no_layernorm=True):
        super(LGEB, self).__init__()
        self.c_weight = c_weight
        n_edge_attr = 2 # dims for Minkowski norm & inner product

        phi_e_layers = OrderedDict([
            ('lin1',  nn.Linear(n_input * 2 + n_edge_attr, n_hidden, bias=False)),
            ('bnorm', nn.BatchNorm1d(n_hidden)),
            ('lnorm', nn.LayerNorm(n_hidden)),
            ('relu1', nn.ReLU()),
            ('lin2',  nn.Linear(n_hidden, n_hidden)),
            ('relu1', nn.ReLU())
        ])
        phi_h_layers = OrderedDict([
            ('lin1',  nn.Linear(n_hidden + n_input + n_node_attr, n_hidden)),
            ('bnorm', nn.BatchNorm1d(n_hidden)),
            ('lnorm', nn.LayerNorm(n_hidden)),
            ('relu1', nn.ReLU()),
            ('lin2',  nn.Linear(n_hidden, n_output))
        ])
        if no_batchnorm:
            phi_e_layers.pop('bnorm')
            phi_h_layers.pop('bnorm')
        if no_layernorm:
            phi_e_layers.pop('lnorm')
            phi_h_layers.pop('lnorm')
...
class LorentzNet(nn.Module):
    ...
    def __init__(self, n_scalar, n_hidden, n_class = 2, n_layers = 6, c_weight = 1e-3, dropout = 0., no_batchnorm=True, no_layernorm=True):
        ...
        self.LGEBs = nn.ModuleList([LGEB(self.n_hidden, self.n_hidden, self.n_hidden, 
                                    ...
                                    c_weight=c_weight, last_layer=(i==n_layers-1), no_batchnorm=no_batchnorm, no_layernorm=no_layernorm)
